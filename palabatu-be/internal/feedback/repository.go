...
func createFeedback(ctx context.Context, userID, email *string, message string, pageURL *string) (*Feedback, error) {
	var f Feedback
	err := db.Pool.QueryRow(ctx, `
		INSERT INTO feedback (user_id, email, message, page_url)
		VALUES ($1, $2, $3, $4)
		RETURNING id, user_id, email, message, page_url, status, created_at
	`, userID, email, message, pageURL).Scan(
		&f.ID, &f.UserID, &f.Email, &f.Message, &f.PageURL, &f.Status, &f.CreatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func listOpenFeedback(ctx context.Context) ([]Feedback, error) {
	rows, err := db.Pool.Query(ctx, `
		SELECT f.id, f.user_id, pr.username, f.email, f.message, f.page_url, f.status, f.created_at
		FROM feedback f
	...
	for rows.Next() {
		var f Feedback
		if err := rows.Scan(&f.ID, &f.UserID, &f.Username, &f.Email, &f.Message, &f.PageURL, &f.Status, &f.CreatedAt); err != nil {
			return nil, err
		}
