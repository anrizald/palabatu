...
**https://palabatu.id is deployed and publicly reachable.** Do not repeat the "not yet deployed" line that used to sit here and still survives in places (ROADMAP.md's Phase 1.5 preamble had it, and so did the since-deleted Railway handoff) — it was already false when those were written, and it licensed some reasoning that no longer holds, most importantly the security bullet in Known WIP rough edges. Verify before restating deploy status, e.g. `curl -sI https://palabatu.id`.

What is actually true, as of 2026-09-06 (deploy status and the empty database re-checked 2026-09-17):
...
- **The deploy docs and config live on `stage` too.** `hostinger_vps_deployment_handoff.md` is the real deploy write-up (13 steps, including the Cloudflare plan in step 10 and the ops chores in step 13), `edge_protection_handoff.md` explains why step 10 isn't optional, and `deploy/compose.yml` + `deploy/Caddyfile` are what the box runs. None of them are on this branch; read them with `git show stage:<path>`.
- **The Railway plan is dead, and its doc is gone.** `railway_prod_deployment_handoff.md` described hosting this on Railway; the VPS won instead, and the file was deleted in `5b7afb6` (`git show 5b7afb6^:railway_prod_deployment_handoff.md` still reads it). Treat it as history, not instructions.
- **`stage` is what the public sees.** It is not an inert holding branch any more. A push doesn't go live by itself, though: nothing in the repo auto-deploys, and a redeploy is a manual `git pull` plus `docker compose up` on the VPS (step 12 of the deploy doc). Whatever is on `stage` goes live at the next redeploy, whoever runs it.
...
- Numbering is at `0025` as of 2026-09-19: `0014`/`0015` the crags/boulders/problems hierarchy, `0016` add-flow v2, `0017` approach guides, `0018` feedback type, `0020` `boulders.filed_uncertain`, `0021` `photo_credits` (see `internal/photocredits` in the Architecture tree), `0022` add-sheet drafts (see `internal/drafts`), `0023` multi-pitch detail (`problems.pitch_count`/`commitment_grade` and `problem_pitches`), `0024` FK indexes (see the indexing rule below), `0025` `problem_high_points` (see `internal/social`).
...
- **Index every new foreign key column, and every column a new query filters or sorts on, in the same migration that adds it.** Postgres indexes only the referenced side of a foreign key, so a new `x_id uuid REFERENCES y(id)` has no index unless you write one. Without it, every lookup by that column scans the whole table, and so does every delete on the parent table (Postgres has to check for referencing rows). That's how 27 of 35 foreign keys ended up unindexed until `0024_fk_indexes` (2026-09-19) covered the eight behind hot queries: the crag list's per-crag counts, a boulder's problems, a problem's comments, profile stats.
  - Match the index to the query, not just the column. A query that filters on one column and sorts by another gets both, in that order: `comments (problem_id, created_at)`, `notifications (user_id, created_at DESC)`. Name it `<table>_<cols>_idx`. A UNIQUE constraint or composite index whose *first* column is the FK already covers it (`sends (problem_id, user_id)` covers `sends.problem_id`, not `sends.user_id`), so don't duplicate it.
  - Drop it in the matching `.down.sql` with `DROP INDEX IF EXISTS`, unless that down file drops the whole table anyway.
  - Plain `CREATE INDEX` is fine while production is empty. Once a table holds real data, plain `CREATE INDEX` blocks writes to it until the build finishes, so use `CREATE INDEX CONCURRENTLY`. That can't run inside a transaction, and `migrate` sends a multi-statement file as one implicit transaction, so each concurrent index needs a migration file of its own.
  - Some foreign keys are unindexed on purpose, so don't "fix" them unprompted: small admin-only tables (`reports`, `boulder_merge_requests`, `boulder_merge_objections`, `feedback`), and columns no query filters on (`crags`/`boulders`/`approaches.created_by`, which are only ever joined *from*, onto `profiles`' primary key; `topo_annotations.updated_by`, `photo_credits.uploaded_by`, `boulders.merged_into`). Index one when a new query starts filtering on it.
  - To list the foreign keys without an index, run this (`t` rows are fine, `f` rows are unindexed; check each against the list above):
    ```sh
    docker exec kepalabatu-postgres-1 psql -U user -d palabatu -c "SELECT c.conrelid::regclass AS tbl, a.attname AS fk_col, EXISTS (SELECT 1 FROM pg_index i WHERE i.indrelid = c.conrelid AND i.indkey[0] = c.conkey[1]) AS indexed FROM pg_constraint c JOIN pg_attribute a ON a.attrelid = c.conrelid AND a.attnum = c.conkey[1] WHERE c.contype = 'f' AND c.connamespace = 'public'::regnamespace ORDER BY indexed, tbl;"
    ```
...
│                            # one-image deploy possible (see the Project section). Mounted as gin's
...
│   │                        # owner.go: RequireOwner; ratelimit.go: RateLimit; bodylimit.go: BodyLimit;
│   │                        # timeout.go: Timeout (the per-request deadline every pgx call inherits)
...
│   ├── crags/                # top level of the crags -> boulders -> problems hierarchy (full design: the removed handoff.md,
│   │   │                     # see the decoder ring below): the place you drive to and park at. Create is open to any
...
│   │   ├── repository.go    # `problems` table queries. Multi-pitch detail (handoff.md open item 14, migrations/
│   │   │                    # 0023): `pitch_count` (NULL is the only way to say single pitch; the CHECK allows 2+)
│   │   │                    # and `commitment_grade` (F/PD/AD/D/TD/ED) are columns on `problems`, and `problem_pitches`
│   │   │                    # holds however many pitches somebody documented, which is deliberately not required to
│   │   │                    # add up to `pitch_count`. Both create and update take the pitches as an array and replace
│   │   │                    # the whole set; an omitted key keeps, `[]` clears. The backend refuses new pitch data on a
│   │   │                    # route whose rock is not a wall (400, never a delete): a route moved onto a boulder keeps
│   │   │                    # its stored pitch detail hidden, so the frontend gates display on the *loaded rock's* type,
│   │   │                    # not `problem.boulder_type`, which goes stale after a move (`lib/pitches.ts`).
│   │   └── errors.go        # ErrNotFound, ErrForbidden, ErrBoulderNotFound, plus the pitch validation errors
...
│   │   │                     # and the private high-point record for a multi-pitch route ("turned back at pitch 4 of
│   │   │                     # 10", migrations/0025 `problem_high_points`, PUT/DELETE /problems/:id/high-point, read back
│   │   │                     # on send-status as `highPoint`). It is its own table and never a `sends` row: six places
│   │   │                     # read a send as "I climbed it" (profile counts and activity, list send_count, the "X sent
│   │   │                     # Y" notification, the developer export and analytics, the purge). Only the owner ever sees
│   │   │                     # it, it notifies no one, and ToggleSend clears it when a send is added.
│   │   ├── repository.go     # `sends` + `comments` + reaction + `problem_high_points` table queries
...
- **Crags/boulders/problems hierarchy** (shipped end to end 2026-08-08 — schema, backend, and frontend; the full design record is the removed `handoff.md`, see the decoder ring in the handoffs bullet below, and `ROADMAP.md`'s Phase 1.5 entry): a problem is the bottom level of `crags -> boulders -> problems` (migrations 0014/0015). A crag is the place you park and walk in from (required `lat`/`lng`, optional `directions`/`access_notes`); a boulder is one rock (optional `lat`/`lng`, owns `image_urls` — the photo(s) every problem on it shares); a problem is one way up that rock, with no location of its own (`crag_id`/`boulder_id` FKs only, `crag_id` denormalized so hot queries skip the two-hop join). Duplicate boulders are a normal state, not a bug: the one-off `cmd/backfill-crags` script gave each pre-existing problem its own boulder (no data said which problems shared a rock), contributors standing at the same rock keep creating more, and the add sheet's "Not sure which one" files one too. They're resolved through `internal/boulders`' merge sub-flow — anyone signed in may suggest "these are the same rock", only the two boulders' own creators may object, only an admin executes the merge (picking which boulder survives), gated by a 48h objection hold an admin can override.
  ...
  - **A rock's own `lat`/`lng` is captured in two places, and is optional everywhere** (built 2026-08-30, closing a gap where the close-zoom rock layer existed but nothing could ever feed it — every add-sheet rock was hardcoded to `lat: null, lng: null`, so only `cmd/backfill-crags`-era rocks ever drew a pin). `components/RockPointMap.tsx` is the shared picker, used by the add sheet's `RockFields` and by `BoulderDetailPage`'s edit form (so rocks added before this can be pinned retroactively). It opens on the parent crag rather than on GPS and never auto-stamps a position — per open item 13, draw the rocks that have a coordinate, never invent one for the rest. It is deliberately *not* a mode of `SpotMiniMap`: same appearance, opposite contract (a spot's pin is required, GPS-first, and warns about other spots 300 m away; a rock's is optional, crag-centred, and warns at 15 m). `PUT /api/boulders/:id` keeps whatever a body leaves out (closed 2026-09-19; it used to overwrite every column, so a body with only `type` wiped the name and rock type, and an empty one cleared the pin and demoted a wall to a boulder). `lat`/`lng` move as a pair: neither key leaves the pin alone, and either one replaces both with what was sent, so an explicit `null` still clears it. `UpdateBoulderRequest.HasCoords`, filled by its `UnmarshalJSON`, is what tells an omitted key from a `null`, since a `*float64` cannot. `name`/`type`/`rock_type` are `*string`, nil meaning keep, and `UpdateBoulder` reads the current row back in for anything omitted, which is also why an untouched NULL name stays NULL. An empty `name` or `rock_type` is a real value (it un-names the rock, and the needs-attention query treats `''` and NULL alike), but an empty `type` means keep, unlike creation. `PUT /api/problems/:id` does the same (2026-09-19): `UpdateProblemRequest`'s text fields are `*string`, `HasHeight` (filled by its `UnmarshalJSON`) separates an omitted `height_m` from an explicit `null`, and an empty `boulder_id` still means keep. It differs in mechanism: `updateProblemRow` keeps omitted columns in the `UPDATE` itself (`COALESCE`, and a `CASE` for the height) instead of reading the row back in, so there is no read-then-write window for a concurrent edit to be lost in, and an untouched NULL stays NULL. The rock's "Move to another spot" and the problem's "Move to another rock" therefore each send just the one id. A move still notifies the creator (`notifyProblemEdited` runs on every update), on purpose. `PUT /api/crags/:id` and `PUT /api/profiles/:id` do the same (2026-09-19), in the same single-statement way. For crags every field is a pointer and `lat`/`lng` are each kept or replaced on their own, since a crag's coordinates are required and there is nothing to clear; `UpdateCrag` range-checks only what was sent. For profiles `title` and `tags` stay `json.RawMessage`, where an omitted key is a nil slice and an explicit `null` is the bytes `"null"`, so a client can still clear either. An omitted `title` is never gated, since nothing about it changes. The profile fix mattered most of the four, because omitted fields used to be written as empty: an admin's body without `title` wiped the role, and one without `avatar_url` counted as a replacement and destroyed the stored Cloudinary avatar. `POST` create paths are unaffected, and every `PUT` in the API now works this way except the two that replace a whole payload by design (`/problems/:id/annotations`, `/drafts/:id`) and `/auth/password`, whose two fields are both required.
  ...
  - **Every design handoff, `handoff.md` itself included, has now shipped
    ...
    - `handoff.md` — the crags/boulders/problems design record: decisions
      1-24, the UX principles, the boulder-merge flow, the data model sketch,
      and open items 1-19. Everything in it is built, or was deliberately
      dropped or left unbuilt inside it (item 19's whole-API partial-update
      convention is the one recommended against). Removed once open item 14,
      the multi-pitch build, finished it. Code comments and this file cite
      "decision N" and "open item N" by those numbers, and they now live only
      in git. Read the last version with
      `git show "$(git log --diff-filter=D -1 --format=%h -- handoff.md)^:handoff.md"`
      (that finds the commit that deleted it and reads the file as it stood
      just before), or list its history with `git log --oneline -- handoff.md`.
- Topo photo annotation (drawing route lines/holds on a boulder's photo, which every problem on that boulder shares — see the hierarchy bullet above): shared frontend components live in `palabatu-fe/src/components/topo-annotations/` (`TopoImage` read-only viewer, `TopoAnnotationEditor` drawing modal, `TopoAnnotationOverlay` the shared SVG renderer used by both, `useContainRect` the geometry hook) and are imported by `ProblemDetailPage.tsx` (`TopoImage`), `BoulderDetailPage.tsx` (`TopoAnnotationOverlay` + `useContainRect`, for the combined every-line-on-this-rock view), and the add sheet's `AddSheet.tsx` (`TopoAnnotationEditor`, opened from `ProblemFields`' photo row and from the post-save banner). **`topo_annotations` was never re-keyed** for the hierarchy change, contrary to what an early draft of `handoff.md` said: its original `(problem_id, image_url)` unique key (migrations/0005) already expresses "one photo, N lines" once the photo belongs to the boulder. The only thing that moved is the membership check — `getProblemOwnerAndBoulderImages` validates a save's URL against the problem's *boulder's* `image_urls`, not the problem's own. Shapes are stored as coordinates normalized to the image's natural width/height (radius/strokeWidth normalized against width for *both* axes, so a circle stays circular regardless of photo aspect ratio) — see `palabatu-fe/src/types/annotation.ts`. A shape may also carry an optional `pitch` (which pitch of a multi-pitch route it belongs to, handoff.md open item 14): it lives inside the same `data` array, so it needed no migration and no backend change (the backend passes `data` through opaquely), and the every-line-on-this-rock view is untouched. Only the single-problem views label lines by pitch (`showPitchLabels`), since the combined view lays several routes' lines over one photo, where each route's own "P1" would mean nothing. `useContainRect` measures the rendered `<img>` box directly via `getBoundingClientRect()` rather than trusting `naturalWidth`/`naturalHeight` math, because those don't reliably match what the browser actually paints for every real-world (often EXIF-oriented) photo.
...
- ~~Dead dependencies in `palabatu-fe/package.json`~~ — cleared 2026-09-06. The markercluster and `@supabase/auth-ui-*` packages this bullet used to name had already gone; five more that nothing imported went with this pass: `cloudinary`, `multer` and `@types/multer` (leftovers from the retired Node/Express backend — image upload and multipart parsing are backend concerns and live in `palabatu-be` now), `rive-js`, and `@tailwindcss/cli` (`vite.config.ts` uses the `@tailwindcss/vite` plugin, so a CLI invocation is nothing's entry point). `npm run build`, `npm run lint` and `tsc --noEmit` all clean afterwards. `npm audit` reports 0 advisories as of 2026-09-19, in both `palabatu-fe` and the root (it was 16 at the time of this pass, all in dev tooling: `vite`'s dev server, `node-tar`).
- ~~**Tailwind is wired up twice**~~ — resolved 2026-09-19. `vite.config.ts`'s `@tailwindcss/vite` plugin is now the only path: `postcss.config.ts`, `tailwind.config.ts` (dead under Tailwind 4, which reads a JS config only through an `@config` directive `index.css` never had) and the `@tailwindcss/postcss`/`postcss`/`autoprefixer` devDependencies are gone. Checked by building before and after: the same 463 class selectors and every custom property survive, and the CSS is about 5 KB smaller. What changed is autoprefixer's work: it no longer adds prefixes for browsers below Tailwind 4's own baseline (`-moz-tab-size`, `-o-object-fit`, `::-moz-placeholder`, `-webkit-appearance`), no longer clones ~50 `::placeholder` rules with their `color-mix` fallbacks, and no longer strips Leaflet's authored legacy prefixes. Don't re-add a PostCSS config "for autoprefixer": Tailwind's Lightning CSS pass already covers its supported browsers.
- The em-dash rule in Copywriting above is now met in `palabatu-fe/src` (cleared 2026-09-19: about 25 user-facing strings across 12 files, plus `index.html` and two notification messages in `internal/notification/service.go`). What remains is code comments, which the rule doesn't cover, and one dev-only console string in `lib/overflowGuard.ts`. Don't add new ones. **Grep for both spellings**, the entity `&mdash;` and a literal `—`, and exclude `//`/`*` comment lines and `types/` before counting, or the number is mostly noise.
- **Backend hardening gaps not yet covered** (see the Backend abuse protection bullet above for what is, including the now-tuned DB pool and body size cap). ~~No per-request query timeout~~, closed 2026-09-19 by `middleware.Timeout` (10s, 30s for multipart), mounted once at the root. It needed no repository changes, because every handler already passes `c.Request.Context()` down and pgx honors it on both `Acquire` and the query. Verified against a locked `crags` table: `GET /api/crags` returned 500 at 10.0s instead of hanging, and the pool recovered. It is a driver-side deadline rather than a server-side `statement_timeout` on purpose: production reaches Neon through PgBouncer (`-pooler`), which can reject unknown startup parameters, and `db.Connect` is fatal on failure. Work that must finish after a commit detaches with `context.WithoutCancel` plus its own bound: `cloudinary.DestroyByURL` (all callers), purge's post-delete bookkeeping, and signup's rollback after a failed verification email. Keep that pattern for any new post-commit cleanup. A related limit is closed too: the server's 30s `WriteTimeout` used to cut off the response of a purge that ran longer than that (mostly its serial Cloudinary destroys, roughly 100 photos), so the admin saw an error for a purge that had succeeded and the automatic snapshot save never fired. `handlePurgeCrag` now calls `http.NewResponseController(c.Writer).SetWriteDeadline` for 5 minutes on that one route (gin's writer supports `Unwrap`, checked with a throwaway server: the plain route's connection was cut at the 1s test timeout, the extended one delivered). `PurgeSpotModal` still offers the snapshot from the preview before purging, and the count echo guarantees it matches. **Network-edge protection is a live gap, not a hypothetical one.** This bullet used to dismiss it as "expected given nothing is deployed yet" — that premise was false (see the Project section) and the conclusion went with it. The current state: a Caddy reverse proxy fronts the app on the VPS and terminates TLS. Its config is `deploy/Caddyfile`, on `stage` only (an earlier version of this bullet said it wasn't in the repo at all). It compresses responses, 404s `/metrics`, and filters nothing else. On 2026-09-17 the box's checkout had no local changes under `deploy/`, so the committed file was what ran; re-check before relying on that. No CDN or WAF is in play: the response headers show none, and `palabatu.id`'s nameservers were still Hostinger's as of 2026-09-17. So the app-level protections above (per-IP rate limiting, body caps, server timeouts) are doing the work, on a publicly reachable origin, and they are explicitly not enough for volumetric/L3-L4 traffic. What limits the damage today is that the deployment holds no data (empty database, under-construction curtain), which is a fact about timing rather than a defense. Putting something with real protections in front is overdue rather than premature, and it can't be solved inside the Go app. The plan is already written: step 10 of `hostinger_vps_deployment_handoff.md` puts Cloudflare's free tier in front, with two changes that must ship together (take `X-Forwarded-For` from `CF-Connecting-IP` in the Caddyfile, and restrict `ufw` to Cloudflare's IP ranges). The Caddyfile already carries the first one, commented out.
